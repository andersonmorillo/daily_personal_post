...
async def register_pet(
    pet: PetCreate, current_user: Annotated[User, Depends(get_current_active_user)]
):
    return JSONResponse(
        content=jsonable_encoder(crud.register_pet(DB, current_user.id, pet)),
        status_code=201,
    )
...
def get_user_info(username: str, db: SessionLocal = Depends(get_db)):
...
def modify_user_info(
    user_update_info: User = get_current_active_user,
    db: SessionLocal = Depends(get_db),
    user_info: User = Depends(get_current_active_user),
):
    return crud.modefy_current_user_info(db, user_info.username, user_update_info)
