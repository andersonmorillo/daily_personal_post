...
class Student(person):
    def __init__(
        self, age, name, lname
    ):  # the method init add new methods to the class strudent, but stop the inherits from the funtion init from the parent
        super().__init__(
            age, name
        )  # super add the properties and methods from the parent
        self.lastname = lname
